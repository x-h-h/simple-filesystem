...
static struct TEST_fs_super_block super_block;
...
static int set_bmap(uint64_t idx, int value)
{
	...
	if(array_idx > bmap_size*TEST_BLOCKSIZE) {
...
static int init_disk(int fd, const char* path)
{
	...
	super_block.block_size = TEST_BLOCKSIZE;
	super_block.magic = MAGIC_NUM;
	super_block.blocks_count = disk_size/TEST_BLOCKSIZE;
	...
	//计算bmap
	bmap_size = super_block.blocks_count/(8*TEST_BLOCKSIZE);
	super_block.bmap_block = RESERVE_BLOCKS;

	if (super_block.blocks_count%(8*TEST_BLOCKSIZE) != 0) {
		bmap_size += 1;
	}
	bmap = (uint8_t *)malloc(bmap_size*TEST_BLOCKSIZE);
	memset(bmap,0,bmap_size*TEST_BLOCKSIZE);

	//计算imap
	imap_size = super_block.inodes_count/(8*TEST_BLOCKSIZE);
	super_block.imap_block = super_block.bmap_block + bmap_size;

	if(super_block.inodes_count%(8*TEST_BLOCKSIZE) != 0) {
		imap_size += 1;
	}
	imap = (uint8_t *)malloc(imap_size*TEST_BLOCKSIZE);
	memset(imap,0,imap_size*TEST_BLOCKSIZE);

	//计算inode_table
	inode_table_size = super_block.inodes_count/(TEST_BLOCKSIZE/TEST_INODE_SIZE);
...
static int write_bmap(int fd) 
{
	...
	ret = write(fd, bmap, bmap_size*TEST_BLOCKSIZE);
	if (ret != bmap_size*TEST_BLOCKSIZE) {
...
static int write_imap(int fd)
{
	...
	ssize_t res = write(fd, imap, imap_size*TEST_BLOCKSIZE);
	if (res != imap_size*TEST_BLOCKSIZE) {
...
static int write_itable(int fd)
{
	...
	ssize_t ret;
	struct TEST_inode root_dir_inode;
	root_dir_inode.mode = S_IFDIR;
	root_dir_inode.inode_no = TEST_ROOT_INODE_NUM;
	...
	struct TEST_inode onefile_inode;
	...
	struct TEST_dir_record root_dir_c;
	const char* cur_dir = ".";
	const char* parent_dir = "..";
	
	
	memcpy(root_dir_c.filename, cur_dir, strlen(cur_dir) + 1);
	root_dir_c.inode_no = TEST_ROOT_INODE_NUM;
	struct TEST_dir_record root_dir_p;
	memcpy(root_dir_p.filename, parent_dir, strlen(parent_dir) + 1);
	root_dir_p.inode_no = TEST_ROOT_INODE_NUM;

	struct TEST_dir_record file_record;
	...
	printf("Current seek is %lu and rootdir at %lu\n", current_off
			, super_block.data_block_number*TEST_BLOCKSIZE);

	if(-1 == lseek(fd, super_block.data_block_number*TEST_BLOCKSIZE, SEEK_SET)) {
		perror("lseek error\n");
		return -1;
	}
...
{
	char dummy[TEST_BLOCKSIZE] = {0};
	ssize_t res = write(fd, dummy, TEST_BLOCKSIZE);
	if (res != TEST_BLOCKSIZE) {
