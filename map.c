...
uint64_t TEST_fs_get_empty_inode(struct super_block* sb)
{
	...
	uint64_t empty_ilock_num = TEST_find_first_zero_bit(imap, disk_sb->blocks_count / 8);
	printk(KERN_INFO "TEST_find_first_zero_bit at %llu\n",
	       empty_ilock_num);
    kfree(imap);
...
int get_imap(struct super_block* sb, uint8_t* imap, ssize_t imap_size)
{
	...
	for (i = disk_sb->imap_block;
		...
		if (imap_size >= TEST_BLOCKSIZE) {
			memcpy(imap, imap_t, TEST_BLOCKSIZE);
			imap_size -= TEST_BLOCKSIZE;
		} else {
...
int get_bmap(struct super_block* sb, uint8_t* bmap, ssize_t bmap_size)
{
	...
	for (i = disk_sb->bmap_block;
		...
		if (bmap_size >= TEST_BLOCKSIZE) {
			memcpy(bmap, bmap_t, TEST_BLOCKSIZE);
			bmap_size -= TEST_BLOCKSIZE;
		} else {
...
uint64_t TEST_fs_get_empty_block(struct super_block* sb)
{
	...
	uint64_t empty_block_num = TEST_find_first_zero_bit(bmap, disk_sb->blocks_count / 8);
	printk(KERN_INFO "TEST_find_first_zero_bit at %llu\n",
	       empty_block_num);
    kfree(bmap);
...
int set_and_save_imap(struct super_block* sb, uint64_t inode_num, uint8_t value)
{
    ...
    struct TEST_fs_super_block *disk_sb = sb->s_fs_info;
    uint64_t block_idx = inode_num / (TEST_BLOCKSIZE*8) + disk_sb->imap_block;
    uint64_t bit_off = inode_num % (TEST_BLOCKSIZE*8);
...
int set_and_save_bmap(struct super_block* sb, uint64_t block_num, uint8_t value)
{
     ...
     */
    struct TEST_fs_super_block *disk_sb = sb->s_fs_info;
    uint64_t block_idx = block_num / (TEST_BLOCKSIZE*8) + disk_sb->bmap_block;
    uint64_t bit_off = block_num % (TEST_BLOCKSIZE*8);
