...
extern struct file_operations TEST_fs_file_ops ;

extern struct file_operations TEST_fs_dir_ops;

extern struct inode_operations TEST_fs_inode_ops;

extern struct address_space_operations TEST_fs_aops;

int TEST_write_inode(struct inode *inode, struct writeback_control *wbc)
{
    struct buffer_head * bh;
	struct TEST_inode * raw_inode = NULL;
    TEST_fs_get_inode(inode->i_sb, inode->i_ino, raw_inode);
	if (!raw_inode)
		return -EFAULT;
...
void TEST_evict_inode(struct inode *vfs_inode)
{
    ...
    {
        printk(KERN_INFO "TEST evict: Inode [%lu] still has links\n", vfs_inode->i_ino);
        return;
    }
    printk(KERN_INFO "TEST evict: Inode [%lu] has no links!\n", vfs_inode->i_ino);
...
{
    return TEST_fs_create_obj(dir, dentry, mode);
}

int TEST_fs_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode)
{
	return TEST_fs_create_obj(dir, dentry, S_IFDIR | mode);
}
...
ssize_t TEST_write_inode_data(struct inode* inode, const void *buf, size_t count)
{
    ...
    if(!buf) {
        printk(KERN_ERR "TEST: buf is null\n");
        return -EFAULT;
    }
    if(count > TEST_BLOCKSIZE*TEST_N_BLOCKS) {
        return -ENOSPC;
    }
    
    if (-1 == TEST_fs_get_inode(sb, inode->i_ino, &H_inode)){
        printk(KERN_ERR "TEST: cannot read inode\n");
		return -EFAULT;
    }
    if(count > TEST_BLOCKSIZE*H_inode.blocks) {
        int ret;
        ret = alloc_block_for_inode(sb, &H_inode, 
                              (count - TEST_BLOCKSIZE*H_inode.blocks) /TEST_BLOCKSIZE);
        if(ret) {
            return -EFAULT;
        }
    ...
    int i;
    i = 0;
    while(count_res && i < TEST_N_BLOCKS) {
        struct buffer_head* bh;
        bh = sb_bread(sb, H_inode.block[i]);
        BUG_ON(!bh);
        size_t cpy_size;
        if(count_res >= TEST_BLOCKSIZE) {
            count_res -= TEST_BLOCKSIZE;
            cpy_size = TEST_BLOCKSIZE;
        }
        else {
            count_res = 0;
            cpy_size = count_res;
        }
        memcpy(bh->b_data, buf+i*TEST_BLOCKSIZE, cpy_size);
...
ssize_t TEST_read_inode_data(struct inode* inode,void* buf, size_t size)
{
	...
	printk(KERN_INFO "TEST: read inode [%llu]\n", inode->i_ino);
	struct TEST_inode H_inode;
	if (-1 == TEST_fs_get_inode(sb, inode->i_ino, &H_inode)){
        printk(KERN_ERR "TEST: cannot read inode\n");
		return -EFAULT;
    ...
    for(i = 0; i < H_inode.blocks; ++i) {
        ...
        bh = sb_bread(sb, H_inode.block[i]);
        BUG_ON(!bh);
        if((i+1)*TEST_BLOCKSIZE > size){
            brelse(bh);
            return i*TEST_BLOCKSIZE;
        }
        memcpy(buf + i*(TEST_BLOCKSIZE), bh->b_data, TEST_BLOCKSIZE);
        brelse(bh);
    }
	return i*(TEST_BLOCKSIZE);
}
...
int TEST_fs_unlink(struct inode *dir, struct dentry *dentry)
{
    ...
    printk(KERN_INFO "TEST: unlink [%s] from dir inode [%lu]\n",
           dentry->d_name.name, dir->i_ino);
    struct TEST_inode H_dir_inode;
    if(TEST_fs_get_inode(sb, dir->i_ino,&H_dir_inode)) {
       return -EFAULT; 
    }
    ssize_t buf_size = H_dir_inode.blocks*TEST_BLOCKSIZE;
    void* buf = kmalloc(buf_size, GFP_KERNEL);
    if(TEST_read_inode_data(dir, buf, buf_size) != buf_size) {
        printk(KERN_ERR "TEST_read_inode_data failed\n");
        kfree(buf);
        return -EFAULT;
    }
    struct inode *inode = d_inode(dentry);
    int i;
    struct TEST_dir_record* p_dir;
    p_dir = (struct TEST_dir_record*) buf;
    for(i = 0; i < H_dir_inode.dir_children_count; ++i) {
        if(strncmp(dentry->d_name.name, p_dir[i].filename, TEST_FILENAME_MAX_LEN)) {
            ...
            //remove it from buf
            struct TEST_dir_record* new_buf = kmalloc(buf_size - sizeof(struct TEST_dir_record), GFP_KERNEL);
            memcpy(new_buf, p_dir, (i)*sizeof(struct TEST_dir_record));
            memcpy(new_buf + i, p_dir + i + 1, 
                       (H_dir_inode.dir_children_count -i- 1)*sizeof(struct TEST_dir_record));
            TEST_write_inode_data(dir, new_buf, buf_size - sizeof(struct TEST_dir_record));
            kfree(new_buf);
            break;
        }
    }
...
int TEST_fs_create_obj(struct inode *dir, struct dentry *dentry, umode_t mode)
{
    ...
    struct TEST_inode H_dir_inode;
    TEST_fs_get_inode(sb, dir->i_ino, &H_dir_inode);
        
    if(H_dir_inode.dir_children_count >= TEST_BLOCKSIZE/sizeof(struct TEST_dir_record)) {
        return -ENOSPC;
    }
    //1. write inode
    uint64_t first_empty_inode_num = TEST_fs_get_empty_inode(dir->i_sb);
    BUG_ON(!first_empty_inode_num);
    struct inode* inode;
    struct TEST_inode raw_inode;
    inode = new_inode(sb);
    if(!inode) {
        return -ENOSPC;
    }
    ...
    if(S_ISDIR(mode)) {
        ...
        struct TEST_dir_record dir_arr[2];
        uint64_t first_empty_block_num = TEST_fs_get_empty_block(sb);
        ...
        save_block(sb, first_empty_block_num, dir_arr, sizeof(struct TEST_dir_record)*2);
        ...
        inode->i_size = 0;
        inode->i_blocks = 0;
        inode->i_fop = &TEST_fs_file_ops;
        inode->i_mapping->a_ops = &TEST_fs_aops;
        raw_inode.blocks = 0;
    ...
    struct TEST_dir_record new_dir;
    memcpy(new_dir.filename, name, strlen(name)+1);
    new_dir.inode_no = first_empty_inode_num;
    struct buffer_head* bh;
    bh = sb_bread(sb, H_dir_inode.block[0]);
    memcpy(bh->b_data + H_dir_inode.dir_children_count*sizeof(struct TEST_dir_record), &new_dir, sizeof(new_dir));
...
int TEST_fs_get_inode(struct super_block *sb,
		      uint64_t inode_no, struct TEST_inode *raw_inode)
{
	if (!raw_inode) {
		printk(KERN_ERR "inode is null");
		return -1;
	}
	struct TEST_fs_super_block *H_sb = sb->s_fs_info;
	struct TEST_inode *H_inode_array = NULL;

	int i;
	struct buffer_head *bh;
	bh = sb_bread(sb,
		      H_sb->inode_table_block
		      + inode_no * sizeof(struct TEST_inode) / TEST_BLOCKSIZE);
	printk(KERN_INFO "H_sb->inode_table_block is %lld",
	       H_sb->inode_table_block);
	BUG_ON(!bh);
	//TODO 
	H_inode_array = (struct TEST_inode *)bh->b_data;
	int idx = inode_no % (TEST_BLOCKSIZE / sizeof(struct TEST_inode));
	ssize_t inode_array_size = TEST_BLOCKSIZE / sizeof(struct TEST_inode);
	if (idx > inode_array_size) {
		printk(KERN_ERR "in get_inode: out of index");
		return -1;
	}
	memcpy(raw_inode, H_inode_array + idx, sizeof(struct TEST_inode));
...
void TEST_fs_convert_inode(struct TEST_inode *H_inode, struct inode *vfs_inode)
{
...
struct dentry *TEST_fs_lookup(struct inode *parent_inode,
			      struct dentry *child_dentry, unsigned int flags)
{
	...
	struct TEST_dir_record *dtptr;
	struct buffer_head *bh;

	printk(KERN_ERR "TEST_fs: lookup [%s] in inode [%lu]\n",
	       child_dentry->d_name.name, parent_inode->i_ino);

	if (-1 == TEST_fs_get_inode(sb, parent_inode->i_ino, &H_inode)){
		printk(KERN_ERR "TEST: cannot get inode\n");
	...
	dtptr = (struct TEST_dir_record *)bh->b_data;
	...
	for (i = 0; i < H_inode.dir_children_count; i++) {
		...
		if (strncmp
			...
			if (!inode) {
				printk(KERN_ERR
				       "TEST_fs lookup: iget_locked() returned NULL\n");
				brelse(bh);
				return ERR_PTR(-EFAULT);
			}
			...
			if (inode->i_state & I_NEW) {
				...
				struct TEST_inode H_child_inode;
				if (-1 == TEST_fs_get_inode(sb, dtptr[i].inode_no, &H_child_inode))
				{
					return ERR_PTR(-EFAULT);
				}

				TEST_fs_convert_inode(&H_child_inode, inode);
...
int save_inode(struct super_block* sb, struct TEST_inode H_inode)
{
    ...
    struct TEST_fs_super_block *disk_sb = sb->s_fs_info;
    uint64_t block_idx = inode_num*sizeof(struct TEST_inode) / TEST_BLOCKSIZE 
        + disk_sb->inode_table_block ;
    uint64_t arr_off = inode_num % (TEST_BLOCKSIZE / sizeof(struct TEST_inode));
    ...
    struct TEST_inode* p_disk_inode;
    p_disk_inode = (struct TEST_inode*)bh->b_data;
